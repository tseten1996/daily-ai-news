...
1a. If this run published a new page (article, module, or index page), run
    `./scripts/generate-sitemap.sh` and commit the resulting `sitemap.xml`.
    `sitemap.xml` is generated, not hand-maintained — never hand-edit it directly.
    `.github/workflows/check-sitemap.yml` fails CI if the committed file drifts from
    the generator's output, so skipping this step leaves a red check on `main` even
    though `deploy-pages.yml` regenerates it again before every deploy.
